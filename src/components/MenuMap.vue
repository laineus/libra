  ...
  <MenuContainer ref="container" :height="390" :title="t('ui.map')">
    ...
    <template v-else>
      <Container v-for="(v, i) in places" :key="i" :x="rowWidth.half + 10" :y="(i * rowHeight) + rowHeight.half + 5" :width="rowWidth" :height="rowHeight" @pointerdown="p => tapItem(p, i)">
        <Rectangle :visible="i === selectedIndex ? !del : gamepadMode && selectedIndex === null && i === cursorIndex && !cursorDel" :fillColor="COLORS.orange" :width="rowWidth" :height="rowHeight" :alpha="0.8" />
        ...
        <Container v-if="v" :width="rowHeight" :height="rowHeight" :x="100" @pointerdown.stop="p => tapGarbage(p, i)">
          <Rectangle :visible="i === selectedIndex ? del : gamepadMode && selectedIndex === null && i === cursorIndex && cursorDel" :fillColor="COLORS.orange" :width="rowHeight" :height="rowHeight" :alpha="0.8" />
          <Image texture="garbage" :scale="0.45" :tint="COLORS.brown" />
        </Container>
      </Container>
      <template v-if="selectedIndex !== null">
        <Selector v-if="del" :x="tapX" :y="tapY" :list="[t('ui.delete'), t('ui.cancel')]" :selectedIndex="gamepadMode ? optionIndex : null" @select="submit" />
        <Selector v-else :x="tapX" :y="tapY" :list="[places[selectedIndex] ? t('ui.moveToRegisteredPlace') : t('ui.registerPlace'), t('ui.cancel')]" :selectedIndex="gamepadMode ? optionIndex : null" @select="submit" />
      </template>
    </template>
  </MenuContainer>
</template>
