...
export default {
  ...
  setup (props, context) {
    ...
    onPreUpdate(() => {
      ...
      const found = field.value.charas.concat(field.value.substances).map(v => Array.isArray(v.ref.value) ? v.ref.value[0] : v.ref.value).filter(v => unref(v?.visible) && v?.hp > 0 && v?.itemData?.damage).find(v => {
